package com.quanminjieshui.waterindex.utils.image;
...
//                .addHeader("token", (String) SPUtils.get(context,HttpConfig.HEAD_TOKEN_KEY,""))
    ...
    public static void clearMemoryCache(){
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                Glide.get(WaterIndexApplication.getInstance()).clearMemory();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(WaterIndexApplication.getInstance()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(WaterIndexApplication.getInstance()).clearDiskCache();
    ...
    public void clearImageAllCache() {
        clearDiskCache();
        clearMemoryCache();
        deleteFolderFile(WaterIndexApplication.getInstance().getExternalCacheDir() + GlideCatchConfig.GLIDE_CARCH_DIR, true);
    }
    ...
    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(WaterIndexApplication.getInstance().getCacheDir() + GlideCatchConfig.GLIDE_CARCH_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
            return "";
        }
